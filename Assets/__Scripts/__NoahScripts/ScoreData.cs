...
public class ScoreData : MonoBehaviour
{
    ...
    public void LoadScoresFromFile()
    {
        ...
        else
        {
            ...
            " does not exist. No scores will be loaded.");
            return;
        }
        ...
        while (fileReader.Peek() != 0 && scoreCount < scores.Length)
        {
            ...
            string fileLineName = fileReader.ReadLine();
            string fileLineScore = fileReader.ReadLine();
            // Try to parse that variable into an int
            // First, make a variable to put it in
            int readScore = -1;
            // Try to parse it
            bool didParse = int.TryParse(fileLineScore, out readScore);
            if (didParse)
            {
                // If we successfully read a number, put it in the array.
                scores[scoreCount] = new Score(fileLineName,readScore);
            }
            else
            {
    ...
    public void SaveScoresToFile()
    {
        ...
        {
            fileWriter.WriteLine(scores[i].name);
            fileWriter.WriteLine(scores[i].score);
        }
        // Close the stream
        fileWriter.Close();
        // Write a log message.
    ...
    public void AddScore(string newName, int newScore)
    {
        ...
        if (desiredIndex < 0)
        {
            ...
            " not high enough for high scores list.");
            return;
        }
        ...
        scores[desiredIndex] = new Score(newName,newScore);
        Debug.Log("Score of " + newScore +
        " entered into high scores at position " + desiredIndex);
    }
}
